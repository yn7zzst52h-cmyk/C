...
int custo( int v[], int n) {
    ...
    int i;
    for(i = 1; i < n; i++) {
        c = c + ( v[i] - v[i-1] ) * ( v[i] - v[i-1] );
    }
    c = c +  ( v[n-1] - v[0] ) * ( v[n-1] - v[0] );
    return c;
}
...
int main()
{
    ...
    int novo[9];
    int melhorsol[9];
    int melhorcust = 99999;

    for(int c = 0; c < 500; c++) {
        copiarvetor(vetor, novo, 9);
        baralharvetor(novo, 9);
        int cn = custo(novo, 9);
        if (cn < melhorcust) {
            melhorcust = cn;
            copiarvetor(novo, melhorsol, 9);
        } 
    }

    printf("Melhor solução encontrada : ");
    mostrarvetor(melhorsol, 9);

    printf("Custo da melhor solução encontrada : %d\n", melhorcust);
